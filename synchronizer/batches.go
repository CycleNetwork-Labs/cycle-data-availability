...
func (bs *BatchSynchronizer) filterEvents() error {
	...
	log.Debugf("filtering events from block number %d to %d", start, end)
...
func (bs *BatchSynchronizer) handleEvent(event *polygonzkevm.PolygonzkevmSequenceBatches) error {
	...
	log.Debugf("handling sequenceBatches event, batch number: %v", event.NumBatch)
...
func (bs *BatchSynchronizer) trySequencer(batch batchKey) *types.OffChainData {
	log.Debugf("trying sequencer %v at %v for batch %v", bs.sequencer.GetAddr(), bs.sequencer.GetUrl(), batch.number)
