...
// InitStartBlock initializes the L1 sync task by finding the inception block for the ZkEVM contract
func InitStartBlock(db *db.DB, l1 config.L1Config) error {
	...
	// if not sync from start, start syncing from current height in order not to sync all offchain data
	if !l1.SyncFromStart {
		eth, err := ethclient.DialContext(ctx, l1.RpcURL)
		if err != nil {
			return err
		}
		...
		err = setStartBlock(db, latestBlock)
		if err != nil {
			return err
		}
		log.Infof("start block set to latest block %d ", latestBlock)
		return nil
	}

	log.Info("starting search for start block of contract ", l1.ZkEVMAddress)
	startBlock, err := findContractDeploymentBlock(ctx, l1.RpcURL, common.HexToAddress(l1.ZkEVMAddress))
	if err != nil {
		return err
	}
	...
	log.Infof("start block set to %d", startBlock.Uint64())
	return nil
}
