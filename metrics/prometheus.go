...
// GaugeVecOpts holds options for the HistogramVec type.
...
// RegisterGaugeVecs registers the provided gauge vecs metrics to the Prometheus
// registerer.
...
// GaugeVec retrieves gauge metric by name
func GaugeVec(name string) (gauge *prometheus.GaugeVec, exist bool) {
	if !initialized {
		return
	}
	...
	if gauge, exist = gaugeVecs[name]; !exist {
		return nil, exist
	}
...
// GaugeVecSet sets the value for gauge with the given name.
func GaugeVecSet(name string, label string, value float64) {
	if !initialized {
		return
	}

	if c, ok := GaugeVec(name); ok {
		c.WithLabelValues(label).Set(value)
	}
}

// GaugeVecSetWithMultiLabel sets the value for gauge with the given name.
func GaugeVecSetWithMultiLabel(name string, label []string, value float64) {
	if !initialized {
		return
	}
...
// GaugeVecInc increments the gauge with the given name.
func GaugeVecInc(name string, label string) {
	if !initialized {
		return
	}

	if g, ok := GaugeVec(name); ok {
		g.WithLabelValues(label).Inc()
	}
}

// GaugeVecDec decrements the gauge with the given name.
...
// registerGaugeIfNotExists registers single gauge vec metric if not exists
...
// unregisterGaugeVecsIfExists unregisters single gauge vec metric if exists
func unregisterGaugeVecIfExists(name string) {
	...
	if gaugeVec, ok = gaugeVecs[name]; !ok {
		log.Warn("Trying to delete non-existing Gauge Vec metrics.")
		return
	}
