...
func Init(cfg Config, lumberjack *lumberjack.Logger) {
	zapLogger, _, err := NewLogger(cfg, lumberjack)
	if err != nil {
		panic(err)
	}
	log = &Logger{x: zapLogger}
...
// should be added at the outputs array. To avoid printing the logs but storing
// them on a file, can use []string{"pathtofile.log"}
func NewLogger(cfg Config, lumberjack *lumberjack.Logger) (*zap.SugaredLogger, *zap.AtomicLevel, error) {
	...
	if lumberjack != nil {
		...
		opts := []zap.Option{}
		fs := make([]zap.Field, 0, len(zapCfg.InitialFields))
		keys := make([]string, 0, len(zapCfg.InitialFields))
		for k := range zapCfg.InitialFields {
			keys = append(keys, k)
		}
		sort.Strings(keys)
		for _, k := range keys {
			fs = append(fs, zap.Any(k, zapCfg.InitialFields[k]))
		}
		opts = append(opts, zap.Fields(fs...))
		logger.WithOptions(opts...)
	} else {
		loggerInner, err := zapCfg.Build()
		if err != nil {
			return nil, nil, err
		}
		logger = loggerInner
